...
class AIDEL {
 public:
  ...
  typedef typename OptimalPiecewiseLinearModel<key_t, size_t>::CanonicalSegment
      canonical_segment;
 ...
 public:
  ...
  ~AIDEL();
  void train(const std::vector<key_t>& keys, const std::vector<val_t>& vals,
             size_t _maxErr);
  void train_opt(const std::vector<key_t>& keys, const std::vector<val_t>& vals,
                 size_t _maxErr);
  ...
  int scan(const key_t& key, const size_t n,
           std::vector<std::pair<key_t, val_t>>& result);
  size_t model_size();

 private:
  size_t backward_train(
