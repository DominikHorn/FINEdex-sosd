...
template <class _Key, class _Data, class _Value = std::pair<_Key, _Data>,
...
class LevelBinCon {
 ...
 public:
  ...
  typedef std::pair<key_type, data_type>
      pair_type;  // maybe different from value_type
 ...
 private:
  struct bin {
    ...
    inline void initialize() { slotuse = 0; }
  ...
  struct root_bin : public bin {
    ...
    inline bool isfull() const { return (bin::slotuse == rootslotmax); }
  };
  struct child_bin : public bin {
    ...
    inline bool isfull() const { return (bin::slotuse == childslotmax); }
  };
 ...
 public:
  ...
  class iterator {
   ...
   public:
    //*** Methods
    inline iterator() : curbin(NULL), curslot(0) {}

    inline iterator(typename LevelBinCon::child_bin* c, unsigned short s)
        : curbin(c), curslot(s) {}

    /// Key of the current slot
    inline const key_type& key() const { return curbin->slotkey[curslot]; }

    /// Writable reference to the current data object
    inline data_type& data() const { return curbin->slotdata[curslot]; }
 ...
 public:
  ...
  inline ~LevelBinCon() { clear(); }

  // =================== the iterator ==================
  inline iterator begin() { return iterator(m_headbin, 0); }
  inline iterator end() {
 ...
 public:
  ...
  void self_check() const {
    if (m_root) {
      ...
      assert(m_headbin == r->children[0] &&
             m_tailbin == r->children[r->slotuse]);
    } else {
      if (m_headbin) {
        child_bin* c = m_headbin;
        for (unsigned int cslot = 1; cslot < c->slotuse; ++cslot) {
 ...
 public:
  inline result_t con_insert_retrain(const key_type& key,
 ...
 private:
  result_t con_insert_start(const key_type& key, const data_type& value) {
    ...
    if (m_root) {
      ...
      // currnode has
      if (!curr->isfull()) {
        std::copy_backward(curr->slotkey + curslot,
                           curr->slotkey + curr->slotuse,
                           curr->slotkey + curr->slotuse + 1);
        std::copy_backward(curr->slotdata + curslot,
                           curr->slotdata + curr->slotuse,
      ...
      if (slot > 0 && root->children[slot - 1]->slotuse < childslotmax) {
        ...
        if (curr->slotkey[0] > key) {
          ...
          std::copy(curr->slotkey + 1, curr->slotkey + curslot, curr->slotkey);
        ...
        std::copy_backward(root->children + slot,
                           root->children + root->slotuse + 1,
        ...
        if (curslot == mid) {
          ...
          std::copy_backward(newchild->slotkey + curslot,
                             newchild->slotkey + newchild->slotuse,
                             newchild->slotkey + newchild->slotuse + 1);
          std::copy_backward(newchild->slotdata + curslot,
                             newchild->slotdata + newchild->slotuse,
      ...
      // not full?
      if (!child->isfull()) {
        std::copy_backward(child->slotkey + slot,
                           child->slotkey + child->slotuse,
                           child->slotkey + child->slotuse + 1);
        std::copy_backward(child->slotdata + slot,
                           child->slotdata + child->slotuse,
  ...
  result_t con_split_child_bin(child_bin* child, size_t slot,
    ...
    if (slot == mid) {
      ...
      slot -= child->slotuse;
      std::copy_backward(newchild->slotkey + slot,
                         newchild->slotkey + newchild->slotuse,
                         newchild->slotkey + newchild->slotuse + 1);
      std::copy_backward(newchild->slotdata + slot,
                         newchild->slotdata + newchild->slotuse,
                         newchild->slotdata + newchild->slotuse + 1);
  ...
  result_t con_insert_start_retrain(const key_type& key,
    ...
    if (m_root) {
      ...
      if (root->children[slot]->slotuse < childslotmax)
        return insert_child_retrain(root->children[slot], key, value, &newkey,
                                    &newchild);
      ...
      if (slot > 0 && root->children[slot - 1]->slotuse < rootslotmax) {
        ...
        if (curr->slotkey[0] > key) {
          ...
          int curslot = find_lower(curr, key);
          // equal?
          if (curslot < curr->slotuse &&
              key_equal(key, curr->slotkey[curslot])) {
            return result_t::failed;
          }
          root->lock();
          prev->lock();
        ...
        result_t r = insert_child_retrain(root->children[slot], key, value,
                                          &newkey, &newchild);
        if (newchild) {
          root->lock();

          std::copy_backward(root->slotkey + slot,
                             root->slotkey + root->slotuse,
                             root->slotkey + root->slotuse + 1);
          std::copy_backward(root->children + slot,
                             root->children + root->slotuse + 1,
                             root->children + root->slotuse + 2);
  ...
  result_t insert_child_retrain(bin* b, const key_type& key,
                                const data_type& value, key_type* newkey,
                                bin** newchild) {
  ...
  void split_child_bin_retrain(child_bin* child, key_type* _newkey,
                               bin** _newchild) {
    assert(child->isfull());
    unsigned int mid = (child->slotuse >> 1);
    child_bin* newchild = allocate_childbin();
  ...
  // ============= scan ==============
 public:
  int scan(const key_type& key, const size_t n,
           std::vector<std::pair<key_type, data_type>>& result) {
    ...
    while (remaining > 0 && child) {
      result.push_back(std::pair<key_type, data_type>(child->slotkey[cslot],
      ...
      if (child->slotuse == 0) {
        ...
        if (unlikely(slot + 1 < root->slotuse))  //chaohong
        {
          std::copy(root->slotkey + slot + 1, root->slotkey + root->slotuse,
                    root->slotkey + slot);
          std::copy(root->children + slot + 1,
                    root->children + root->slotuse + 1, root->children + slot);
        }
