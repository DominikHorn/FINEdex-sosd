...
template <class key_t, class val_t>
inline AIDEL<key_t, val_t>::AIDEL(int _maxErr, int _learning_step,
...
void AIDEL<key_t, val_t>::train(const std::vector<key_t>& keys,
  ...
  while (start < end) {
    ...
    // equal
    if (err == maxErr) {
      append_model(model, keys.begin() + start, vals.begin() + start,
                   end - start, err);
    } else if (err < maxErr) {
      if (end >= keys.size()) {
        append_model(model, keys.begin() + start, vals.begin() + start,
      ...
      size_t offset =
          backward_train(keys.begin() + start, vals.begin() + start,
                         end - start, int(learning_step * learning_rate));
      end = start + offset;
    }
    start = end;
    end += learning_step;
...
template <class key_t, class val_t>
size_t AIDEL<key_t, val_t>::backward_train(
...
template <class key_t, class val_t>
void AIDEL<key_t, val_t>::append_model(
    lrmodel_type& model,
    const typename std::vector<key_t>::const_iterator& keys_begin,
...
template <class key_t, class val_t>
typename AIDEL<key_t, val_t>::aidelmodel_type* AIDEL<key_t, val_t>::find_model(
    const key_t& key) {
  // root
  size_t model_pos =
...
// =================  scan ====================
template <class key_t, class val_t>
int AIDEL<key_t, val_t>::scan(const key_t& key, const size_t n,
...
// =================== insert the data =======================
template <class key_t, class val_t>
inline result_t AIDEL<key_t, val_t>::insert(const key_t& key,
                                            const val_t& val) {
...
// ================ update =================
template <class key_t, class val_t>
inline result_t AIDEL<key_t, val_t>::update(const key_t& key,
                                            const val_t& val) {
