...
template <class key_t>
void LinearRegressionModel<key_t>::train(
    const typename std::vector<key_t>::const_iterator& it, size_t size) {
...
void LinearRegressionModel<key_t>::train(const std::vector<key_t>& keys,
  ...
  double x_expected = 0, y_expected = 0, xy_expected = 0, x_square_expected = 0;
...
void LinearRegressionModel<key_t>::print_weights() const {
  std::cout << "Weight[0]: " << weights[0] << " ,weight[1]: " << weights[1]
            << std::endl;
}
...
template <class key_t>
std::vector<size_t> LinearRegressionModel<key_t>::predict(
