...
template <class key_t, class val_t>
inline FINEdex<key_t, val_t>::FINEdex(int _maxErr, int _learning_step,
                                      float _learning_rate)
    : maxErr(_maxErr),
      learning_step(_learning_step),
...
void FINEdex<key_t, val_t>::train(const std::vector<key_t>& keys,
  ...
  maxErr = _maxErr;
  std::cout << "training begin, length of training_data is:" << keys.size()
  ...
  auto build_level = [&](auto epsilon, auto in_fun, auto out_fun) -> size_t {
    ...
    auto n_segments =
        make_segmentation_data(last_n, epsilon, in_fun, out_fun, lr);
    return n_segments;
  };

  // Build first level
  auto in_fun = [&](auto i) {
    return std::pair<key_t, size_t>(keys[i], i);
  };
  auto out_fun = [&](auto model, auto keys_begin, auto vals_begin, auto size,
                     auto err) {
...
template <class key_t, class val_t>
void FINEdex<key_t, val_t>::append_model(
    lrmodel_type& model,
    const typename std::vector<key_t>::const_iterator& keys_begin,
    const typename std::vector<val_t>::const_iterator& vals_begin, size_t size,
    int err) {
...
template <class key_t, class val_t>
typename FINEdex<key_t, val_t>::aidelmodel_type*
FINEdex<key_t, val_t>::find_model(const key_t& key) {
  // root
  size_t model_pos =
      binary_search_branchless(&model_keys[0], model_keys.size(), key);
...
// =================  scan ====================
template <class key_t, class val_t>
int FINEdex<key_t, val_t>::scan(const key_t& key, const size_t n,
...
// =================== insert the data =======================
template <class key_t, class val_t>
inline result_t FINEdex<key_t, val_t>::insert(const key_t& key,
                                              const val_t& val) {
...
// ================ update =================
template <class key_t, class val_t>
inline result_t FINEdex<key_t, val_t>::update(const key_t& key,
                                              const val_t& val) {
