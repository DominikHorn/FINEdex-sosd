...
class LevelIndex {
 ...
 private:
  void onelevel(size_t hang) {
    ...
    COUT_THIS("[Root-1level] capacity+hang:" << capacity + hang
                                             << " ,top_size: " << top_size);
  ...
  void twolevel(size_t hang) {
    top_size =
        (int)std::ceil((capacity + hang) / double(page_size * page_size));
  ...
  size_t find_2level(const key_t key) {
    ...
    //memory_fence();
    int j =
        linear_search_avx(tables + top_size + i * page_size, page_size, key);
 ...
 public:
  explicit TwoLevelIndex(std::vector<key_t>& data, int k2, int page_size)
    ...
    for (int i = 0; i < k1_ * k2_; i++) {
      tables_[k1_ + i] = ((i + 1) * page_size_ > data.size())
                             ? LONG_MAX
                             : data[(i + 1) * page_size_ - 1];
    }
    // populate top level index
    for (int i = 0; i < k1_; i++) {
...
class ThreeLevelIndex {
 ...
 public:
  explicit ThreeLevelIndex(std::vector<int>& data, int k2, int k3,
                           int page_size)
    ...
    for (int i = 0; i < k1_ * k2_ * k3_; i++) {
      table3_[i] = ((i + 1) * page_size_ > data.size())
                       ? INT_MAX
                       : data[(i + 1) * page_size_ - 1];
    }
